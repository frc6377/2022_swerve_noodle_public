...
import frc.robot.systems.drivetrain.config.PodConstants.PodConstantsBuilder;
  ...
  private static void init(){
    ...
    Pods.addPod(
      ...
      new PodConstantsBuilder()
        .setEncoderID(1)
        .setEncoderOffset(-Math.toRadians(55.0)));

    Pods.addPod(
      PodName.B,
      new PodConstantsBuilder()
        .setEncoderID(3)
        .setEncoderOffset(-Math.toRadians(108.0)));
    
    Pods.addPod(
      PodName.C,
      new PodConstantsBuilder()
        .setEncoderID(5)
        .setEncoderOffset(Math.toRadians(189.0)));
    
    Pods.addPod(
      PodName.D,
      new PodConstantsBuilder()
        .setEncoderID(7)
        .setEncoderOffset(-Math.toRadians(196.0)));
  }
